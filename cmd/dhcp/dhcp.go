...
func applyLease(nl *netlink.Handle, ifname string, source string, lease dhcp4.Lease, ntp []net.IP, extraRoutePriority int) error {
	...
	if len(ntp) > 0 {
		details = append(details, fmt.Sprintf("NTP %v", ntp))
	}
	...
	if ntp != nil {
		if err := writeNTPServers(ntp); err != nil {
			return fmt.Errorf("writing ntp-servers: %v", err)
		}
	}
...
func ntpServersFromACK(ack *layers.DHCPv4) []net.IP {
	servers := make([]net.IP, 0, 4)
	for _, opt := range ack.Options {
		if opt.Type != layers.DHCPOptNTPServers {
			continue
		}
		for b := opt.Data; len(b) >= 4; b = b[4:] {
			servers = append(servers, net.IP(b[:4]))
		}
	}
	return servers
}

func writeNTPServers(servers []net.IP) error {
	const ntpServers = "/tmp/ntp-servers"

	if len(servers) == 0 {
		if err := os.Remove(ntpServers); err != nil && !os.IsNotExist(err) {
			return fmt.Errorf("removing %s: %v", ntpServers, err)
		}
		return nil
	}
	...
	for _, srv := range servers {
		lines = append(lines, srv.String())
	}
	if err := renameio.WriteFile(ntpServers, []byte(strings.Join(lines, "\n")+"\n"), 0644); err != nil {
		return fmt.Errorf("ntp-servers: %v", err)
	}
	return nil
}
...
func main() {
	...
	if *staticConfig != "" {
		...
		if err := applyLease(nl, *ifname, lsrc, l, nil, *extraRoutePriority); err != nil {
			log.Fatal(err)
		}
	...
	for {
		...
		if err := applyLease(nl, *ifname, "DHCPACK", lease, ntpServersFromACK(last), *extraRoutePriority); err != nil {
			log.Fatal(err)
		}
